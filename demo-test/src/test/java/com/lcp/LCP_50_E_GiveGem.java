...
*/
public class LCP_50_E_GiveGem {

    @Test
    public void test() {
        assert 2 == giveGem(new int[]{3, 1, 2}, new int[][]{{0, 2}, {2, 1}, {2, 0}});
        assert 75 == giveGem(new int[]{100, 0, 50, 100}, new int[][]{{0, 2}, {0, 1}, {3, 0}, {3, 0}});
        assert 0 == giveGem(new int[]{0, 0, 0, 0}, new int[][]{{1, 2}, {3, 1}, {1, 2}});
    }

    public int giveGem(int[] gem, int[][] operations) {
        for(int[] opt : operations) {
            int send = gem[opt[0]] >> 1;
            gem[opt[0]] -= send;
            gem[opt[1]] += send;
        }
        return Arrays.stream(gem).max().getAsInt() -
               Arrays.stream(gem).min().getAsInt();
    }
