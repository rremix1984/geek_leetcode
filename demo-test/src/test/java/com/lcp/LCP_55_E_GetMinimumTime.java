...
/**
    ...
    提示：
        ...
        1 <= limit <= 100
*/
public class LCP_55_E_GetMinimumTime {

    @Test
    public void test() {
        assert 10 == getMinimumTime(new int[]{2, 3, 2},  new int[][]{{0, 2}, {1, 4}, {2, 1}}, 3);
        assert 5 == getMinimumTime(new int[]{1},  new int[][]{{0, 3}, {0, 5}}, 2);
    }
    ...
    public int getMinimumTime(int[] time, int[][] fruits, int limit) {
        ...
        for (int[] arr : fruits) {
            ...
            if (arr[1] % limit == 0)
                sum += (arr[1] / limit) * time[arr[0]];
            else
                sum += (arr[1] / limit + 1) * time[arr[0]];
        }
        return sum;
    }
