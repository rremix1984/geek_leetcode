...
/**
    ...
    示例 1：
        ...
        image.png
    示例 2：
        输入：temperatureA = [5,10,16,-6,15,11,3]
...
*/
public class LCP_61_E_TemperatureTrend_x2 {

    @Test
    public void test() {
        assert 2 == temperatureTrend(
                new int[]{21, 18, 18, 18, 31},
                new int[]{34, 32, 16, 16, 17});
        assert 3 == temperatureTrend(
    ...
    public int temperatureTrend(int[] a, int[] b) {
        ...
        for (int i = 0; i < a.length - 1; i++)
            if (compare(a[i], a[i + 1]) != compare(b[i], b[i + 1]))
                cnt = 0;
            else
                max = max(max, ++cnt);
        return max;
