...
/**
    ...
    方法一：贪心
        思路与算法
        首先很容易得出：从某一个偶（奇）数位置 pi 改变到另一个偶（奇）数位置 pj，不妨设 pi < pj，那么一定 \exists k \in \mathbb{N}^*∃k∈N∗
        使得 pi + 2k = pj 成立，即此时的最小开销为 0。
        从某一个偶（奇）数位置 pi 改变到另一个奇（偶）数位置 pj ，不妨设 pi < pj，那么一定 \exists k \in \mathbb{N}∃k∈N 使得 pi + 2k + 1 = pj成立，即此时的最小开销为 1。
