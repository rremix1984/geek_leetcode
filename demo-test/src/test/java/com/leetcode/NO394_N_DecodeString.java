...
public class NO394_N_DecodeString {
    ...
    public String decodeString(String s) {
        ...
        int multi = 0;
        // 数字队列，19, 31, 51 ...
        Deque<Integer> queue_multi = new LinkedList<>();
        // 括号 [] 队列   [abc] ... [bb] ...  [xyz]
        ...
        for(Character c : s.toCharArray()) {
            // 第 1 种场景，遇到左括号 "["
            if(c == '[') {
                queue_multi.addLast(multi);
                queue.addLast(res.toString());
                multi = 0;
                res.setLength(0);
            // 第 2 种场景，遇到右括号 "]"
            ...
            // 把结果放入 res 中
                ...
                // 获取前面的计数器值
                int size = queue_multi.removeLast();
                // 重复 size 遍 res 值
                for(int i = 0; i < size; i++)
                    tmp.append(res);
                // res 一旦被重复过 res 就不需要了，所以每次都是 new 出来的
                res = new StringBuilder(queue.removeLast() + tmp);
            // 遇到数字,需要对multi变量做累加
            } else if(c >= '0' && c <= '9') {
