...
public class MatrixNode<T> {
    ...
    public static <T> MatrixNode<T> init(int row, int col, Supplier<T> supplier) {
        MatrixNode<T> dummy = new MatrixNode<T>();
        dummy.setVal(supplier.get());
        MatrixNode<T> row1 = dummy;
        for (int i = 1; i < row; i++) {
            row1.right = new MatrixNode<T>();
            row1.right.setVal(supplier.get());
        ...
        for (int i = 1; i < row; i++) {
            MatrixNode<T> newHead = new MatrixNode<T>();
            newHead.setVal(supplier.get());
            newHead.up = pre;
            pre.down = newHead;
            ...
            for (int j = 1; j < col; j++) {
                right.right = new MatrixNode<T>();
                right.right.setVal(supplier.get());
