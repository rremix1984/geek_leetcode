...
/**
    ...
    方法一：贪心
        思路与算法
    首先很容易得出：
    一、从某一个偶（奇）数位置 pi 改变到另一个偶（奇）数位置 pj，
        不妨设 pi < pj，那么一定 ∃ k ∈ N∗
        使得 pi + 2k = pj 成立，即此时的最小开销为0。
    二、从某一个偶（奇）数位置 pi 改变到另一个奇（偶）数位置 pj ，不妨设 pi < pj，
        那么一定 ∃k ∈ N 使得 pi + 2k + 1 = pj 成立，即此时的最小开销为 1。
    三、那么我们可以把初始每一个偶数位置的「筹码」看作一个整体，每一个奇数位置的「筹码」看作一个整体。
       因为我们的目标是最后将全部的「筹码」移动到同一个位置，
    四、那么最后的位置只有两种情况：
