...
public class ReEntryLockDemo {
    public synchronized void m1() {
        ...
        m2();
        out.println(Thread.currentThread().getName() + "\t ----end m1");
    }
    ...
    public static void main(String[] args) {
        ...
        new Thread(() -> {
            lock.lock();
            try {
                out.println(Thread.currentThread().getName() + "\t ----come in外层调用");
                lock.lock();
                try {
                    out.println(Thread.currentThread().getName() + "\t ----come in内层调用");
                } finally {
                    lock.unlock();
                }
