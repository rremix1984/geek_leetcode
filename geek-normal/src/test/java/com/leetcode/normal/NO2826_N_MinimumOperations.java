...
/**
    ...
    思路和算法：
        由于 n≥1，任意一个长度为 1 的子序列都是非严格递增子序列，因此数组 nums 中的以任意一个下
    标结尾的最长非严格递增子序列长度都大于等于 1。为了得到最长非严格递增子序列的长度，对于每个 0≤i<n
    需要分别计算以下标 i 结尾的最长非严格递增子序列长度，然后在这 n 个最长非严格递增子序列的长度中寻找
    整个数组的最长非严格递增子序列长度，即可得到答案。
     1）当 i>0 时，如果存在下标 j 满足 0≤j<i 且 nums[j]≤nums[i]，则可以将 nums[i] 添加到以
        nums[j] 结尾的非严格递增子序列的后面，得到新的非严格递增子序列。因此可以使用动态规划计算
        以每个下标结尾的最长非严格递增子序列长度。创建长度为 n 的数组 dp，其中 dp[i] 为以下标 i 结尾
        的最长非严格递增子序列长度。由于以任意一个下标结尾的最长非严格递增子序列长度都大于等于 1，
        因此将 dp 中的所有值初始化为 1。
     2）当 i=0 时，以下标 i 结尾的子序列只有一个，长度为 1，因此动态规划的边界情况是 dp[0]=1。
     3）当 i>0 时，对于满足 0≤j<i 且 nums[j]≤nums[i] 的任意下标 j，dp[i]≥dp[j]+1，为了使
        dp[i] 最大化，应寻找符合要求的最大的 dp[j]，此时 dp[i]=max{dp[j]}+1。
    因此动态规划的状态转移方程是：对于所有满足 0≤j<i 且 nums[j]≤nums[i] 的下标 j，
    dp[i]=max{dp[j]}+1。由于每一项依赖于之前的项，因此应从小到大遍历每个 i 并计算 dp[i]。
    计算得到 dp 中的所有状态值之后，其中的最大值即为最长非严格递增子序列的长度。
