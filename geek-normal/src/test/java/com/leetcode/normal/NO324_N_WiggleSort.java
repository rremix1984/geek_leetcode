...
/**
    ...
    题解：
        不考虑进阶要求的时间O(N)，空间O(1)，用基础的排序法也是能做本题的。
        先把数组复制，从大到小排序。
        然后按照先奇数角标，后偶数角标的顺序，把从大到小的数据回填，即可完成题目要求。
        可以看到，按照上述方法调整顺序后，必然满足以下条件：
          1）偶数角标是不增序列
          2）奇数位置均大于等于偶数位置
        由于题目默认答案可构造，因此上述方法调整后一定满足题意。
        时间O(N*logN)：排序的时间复杂度，空间O(N)：数组复制占用空间
*/
