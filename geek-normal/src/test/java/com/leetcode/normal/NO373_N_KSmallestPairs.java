...
public class NO373_N_KSmallestPairs {
    ...
    public List<List<Integer>> kSmallestPairs(int[] nums1, int[] nums2, int k) {
        // 2024/3/13 NO.1
        // 2024/3/23 NO.2
        return kSmallestPairs(true, nums1, nums2, k);
    }
    public List<List<Integer>> kSmallestPairs(boolean flag, int[] nums1, int[] nums2, int k) {
        int n = nums1.length;
        int m = nums2.length;
        // 判断是否需要交换顺序
        if (n <= m || (flag = false)) {// 注意：队列中存储的只是下标
            ...
            return kSmallestPairs(flag, nums2, nums1, k);
        }
    }
