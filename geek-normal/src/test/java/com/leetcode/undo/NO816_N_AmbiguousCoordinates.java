...
/**
    ...
    816. 模糊坐标
        ...
        原始的坐标表示法不会存在多余的零，所以不会出现类似于"00", "0.0",
        "0.00", "1.0", "001", "00.01"或一些其他更小的数来表示坐标。
...
*/
public class NO816_N_AmbiguousCoordinates {

    @Test
    public void test() {
        assert getArray("(1, 23)", "(1, 2.3)", "(12, 3)", "(1.2, 3)").equals(
            ambiguousCoordinates("(123)"));
        assert getArray("(0, 0.011)","(0.001, 1)").equals(
            ambiguousCoordinates("(00011)"));
        assert getArray("(0, 123)", "(0, 1.23)", "(0, 12.3)", "(0.1, 23)", "(0.1, 2.3)", "(0.12, 3)").equals(
            ambiguousCoordinates("(0123)"));
        assert getArray("(10, 0)").equals(
